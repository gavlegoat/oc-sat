...
let collect_clause_literals (cl : clause) : varset =
  LSet.of_list (List.map base_literal cl.li
...
let strip_satisfied_clauses (form : formula) : formula =
  { clauses = List.filter (fun c -> not (clause_satisfied c)) form.clauses }
...
let find_unit_clause (f : formula) : clause option =
  List.find_opt (fun c -> Option.is_some (get_unit_literal c)) f.clauses
