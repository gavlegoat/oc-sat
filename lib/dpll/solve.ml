...
(** Replace a variable with a concrete value in a formula.
    ...
    [form] with the boolean value [value], and each instance of [neg lit] with
    [not value]. *)
...
let rec propagate_unit_clauses (st : sat_state) : sat_state =
  ...
  | Some c ->   (* There is a unit clause, assign it and recur. *)
...
let rec recurse (cfg : sat_config) (st : sat_state) : sat_result =
  ...
  then make_sat_result true (Some st2.interpretation)
  else
    if formula_unsatisfiable st2.formula
    then make_sat_result false None
    else
